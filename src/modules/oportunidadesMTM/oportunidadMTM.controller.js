...
import {
  mtmFrontendLink,
  buildVariablesPlantillaMtmEstudianteOportunidad,
...
async function dispatchMonitoriaNotificacion(eventValue, datos, recipientContext, metadataExtra = {}) {
  try {
    ...
    console.error(`[MTM] Notificación ${eventValue}:`, err);
  }
}
...
/**
 * La columna "programa" del admin puede concatenar varios nombres con coma.
 * Para filtros se tratan como tokens individuales (unificados en meta y búsqueda por uno).
 */
function tokensProgramaColumnaMTMAdmin(programaCell) {
  if (programaCell == null || String(programaCell).trim() === "") return [];
  return [
    ...new Set(
      String(programaCell)
        .split(",")
...
/** Legalización MTM antes de revisión (`creada`, legado `borrador`) o en ajuste. */
function legalizacionMtmEditablePorEstudiante(est) {
 ...
 * el resto son nombres. 1 palabra → solo nombre; 2 → nombre + apellido.
 */
function nombresYApellidosDesdeNombreCompleto(fullName) {
...
export const changeStatusMTM = async (req, res) => {
  try {
    ...
    const VALID_STATES = ["Creada", "Activa", "Inactiva"];
...
export const getMisAceptadasMTM = async (req, res) => {
  try {
    ...
    const data = list.map((p) => {
      ...
      const { nombre: nombreAlumno, apellido: apellidoAlumno } = nombresYApellidosDesdeNombreCompleto(nombreCompleto);
      return {
        _id: p._id,
        oportunidadId: opp?._id,
        ...
        estado: "Aceptado",
        /** Código del modelo LegalizacionMTM (creada | en_revision | aprobada | rechazada | en_ajuste); null si aún no existe registro de legalización */
...
export const cerrarOportunidadMTM = async (req, res) => {
  try {
    ...
    for (const p of postulaciones) {
      const idStr = String(p._id);
      const fueSeleccionado = selectedIds.includes(idStr);
      const ctx = await loadMtmPostulacionContext(p._id);
      if (!ctx) continue;
      const eventValue = fueSeleccionado
        ? "aprobacion_postulante_por_oportunidad"
        : "rechazo_postulante_por_oportunidad";
      await dispatchMonitoriaNotificacion(eventValue, ctx.datos, {
    ...
    if (accion === "confirmar") {
      ...
      if (ctx) {
        await dispatchMonitoriaNotificacion(
          "aceptacion_oferta_por_estudiante",
          ctx.datos,
          recipientContextCoordProf(ctx.profEmail)
        );
      }
    } else {
      const ctx = await loadMtmPostulacionContext(postulacionId);
...
export const updateApplicationStateMTM = async (req, res) => {
  try {
    ...
    if (estado === "rechazado") {
      const ctx = await loadMtmPostulacionContext(postulacionId);
      if (ctx) {
        await dispatchMonitoriaNotificacion("rechazo_postulante_por_oportunidad", ctx.datos, {
          estudiante: ctx.estudianteEmail,
          postulante: ctx.estudianteEmail,
        });
      }
...
export const updateLegalizacionMTM = async (req, res) => {
  try {
    ...
    if (!legalizacionMtmEditablePorEstudiante(leg.estado)) {
      return res.status(400).json({ message: "Solo se puede editar en estado creada o en ajuste" });
    }
...
export const uploadDocLegalizacionMTM = async (req, res) => {
  try {
    ...
    let leg = await LegalizacionMTM.findOne({ postulacionMTM: postulacionId });
    if (!leg) {
      leg = await LegalizacionMTM.create({ postulacionMTM: postulacionId, estado: "creada" });
    }
    if (!legalizacionMtmEditablePorEstudiante(leg.estado)) {
      return res.status(400).json({ message: "Solo se puede subir documentos en estado creada o en ajuste" });
    }
...
export const deleteDocumentoLegalizacionMTM = async (req, res) => {
  try {
    ...
    if (!leg) return res.status(404).json({ message: "Legalización no encontrada" });
    if (!legalizacionMtmEditablePorEstudiante(leg.estado)) {
      return res.status(400).json({ message: "Solo se puede eliminar documentos en estado creada o en ajuste" });
    }
...
export const remitirRevisionLegalizacionMTM = async (req, res) => {
  try {
    ...
    if (!leg) return res.status(404).json({ message: "Legalización no encontrada" });
    if (!legalizacionMtmEditablePorEstudiante(leg.estado)) {
      return res.status(400).json({ message: "Solo se puede remitir desde estado creada o en ajuste" });
    }
    ...
    const ctxLeg = await loadMtmPostulacionContext(postulacionId);
    if (ctxLeg) {
      await dispatchMonitoriaNotificacion(
        "envio_revision_legalizacion_monitoria",
        ctxLeg.datos,
        {
          coordinador: [ctxLeg.profEmail, ...parseEnvEmailList(process.env.NOTIFICATION_EMAILS_COORDINADOR)].filter(
            Boolean
          ),
...
/** Filas enriquecidas para listado admin (sin paginar ni filtrar por periodo/programa/búsqueda). */
async function buildLegalizacionesMTMAdminListRows(mongoFilter = {}) {
  const legs = await LegalizacionMTM.find(mongoFilter)
    .populate({
      path: "postulacionMTM",
  ...
  const postulacionIds = postulacionesValidas.map((l) => l.postulacionMTM._id).filter(Boolean);
  const planesCoordinador =
    postulacionIds.length > 0
      ? await PlanDeTrabajoMTM.find({ postulacionMTM: { $in: postulacionIds } })
          .select("postulacionMTM coordinadorMonitoria")
          .lean()
      : [];
  const coordinadorPorPostulacion = new Map();
  for (const pl of planesCoordinador) {
    const k = String(pl.postulacionMTM);
    const texto = (pl.coordinadorMonitoria || "").trim();
    if (!texto) continue;
    if (!coordinadorPorPostulacion.has(k)) coordinadorPorPostulacion.set(k, texto);
  }

  const list = postulacionesValidas.map((l) => {
    ...
    const desdeRef = opp?.profesorResponsable
      ? [opp.profesorResponsable.nombres, opp.profesorResponsable.apellidos].filter(Boolean).join(" ").trim()
      : "";
    const desdeNombreLibre = (opp?.nombreProfesor && String(opp.nombreProfesor).trim()) || "";
    const desdePlanMigrado = coordinadorPorPostulacion.get(String(po?._id)) || "";
    const coordinador = desdeRef || desdeNombreLibre || desdePlanMigrado || null;
    ...
    return {
      ...
      coordinador,
      estadoAlumnoMTM: po?.estado ?? null,
      estadoMTM:
...
function mongoFilterEstadoLegalizacionAdmin(estado) {
  const filter = {};
  if (!estado || String(estado).trim() === "") return filter;
  const e = String(estado).trim();
  /** Compat: valores de UI antigua; en BD `LegalizacionMTM.estado` es creada|borrador|en_revision|aprobada|… */
  const estadoDoc = { legalizada: "aprobada", anulada: "rechazada" }[e];
  if (e === "aceptada") {
    filter.estado = { $in: ["creada", "borrador"] };
  } else if (estadoDoc) {
    filter.estado = estadoDoc;
  } else {
    filter.estado = e;
  }
  return filter;
  ...
  try {
    ...
    if (postIds.length > 0) {
      ...
      if (oppIds.length > 0) {
        ...
        periodos = [
          ...new Set(
            opps.map((o) => (o.periodo?.codigo != null ? String(o.periodo.codigo).trim() : "")).filter((c) => c !== "")
          ),
        ].sort((a, b) => String(a).localeCompare(String(b), "es"));
      }
    }
...
export const getLegalizacionesMTMAdmin = async (req, res) => {
  try {
    const { estado, periodo, programa, page = 1, limit = 20, search = "" } = req.query;
    const mongoFilter = mongoFilterEstadoLegalizacionAdmin(estado);
    let { list } = await buildLegalizacionesMTMAdminListRows(mongoFilter);

    if (periodo != null && String(periodo).trim() !== "") {
      const p = String(periodo).trim();
      list = list.filter((r) => String(r.periodo ?? "") === p);
    }
    if (programa != null && String(programa).trim() !== "") {
      const pr = String(programa).trim();
      list = list.filter((r) => tokensProgramaColumnaMTMAdmin(r.programa).includes(pr));
    }
    const q = String(search ?? "").trim().toLowerCase();
    if (q) {
      list = list.filter((r) => {
        const fields = [r.numeroIdentidad, r.nombre, r.apellido, r.programa, r.codigoMTM, r.nombreMTM, r.coordinador];
    ...
    res.json({ data: list, total, page: pageNum, limit: limitNum, totalPages });
  } catch (err) {
    ...
    if (estadoDocumento === "rechazado") {
      ...
      if (ctxDoc) {
        ...
        await dispatchMonitoriaNotificacion(
          ...
          {
            ...ctxDoc.datos,
            NOMBRE_DOCUMENTO: labelDoc,
            COMENTARIO: motivo,
            OBSERVACION: motivo,
          },
          { estudiante: ctxDoc.estudianteEmail, postulante: ctxDoc.estudianteEmail }
        );
      }
    }
    const updated = await LegalizacionMTM.findById(leg._id).populate("eps tipoCuenta banco", "value description listId").lean();
...
export const postAprobarLegalizacionMTM = async (req, res) => {
  try {
    ...
    await leg.save();
    const ctxApr = await loadMtmPostulacionContextFromLegalizacion(leg._id);
    if (ctxApr) {
      await dispatchMonitoriaNotificacion("aprobacion_legalizacion_monitoria", ctxApr.datos, {
        estudiante: ctxApr.estudianteEmail,
        postulante: ctxApr.estudianteEmail,
      });
    }
    const updated = await LegalizacionMTM.findById(leg._id).populate("eps tipoCuenta banco", "value description listId").lean();
...
export const postRechazarLegalizacionMTM = async (req, res) => {
  try {
    ...
    await leg.save();
    const ctxRec = await loadMtmPostulacionContext(postulacionId);
    if (ctxRec) {
      const textoMotivo = motivoStr || "";
      await dispatchMonitoriaNotificacion(
        "rechazo_legalizacion_monitoria",
        { ...ctxRec.datos, COMENTARIO: textoMotivo, OBSERVACION: textoMotivo },
        { estudiante: ctxRec.estudianteEmail, postulante: ctxRec.estudianteEmail }
      );
    }
    const updated = await LegalizacionMTM.findById(leg._id).populate("eps tipoCuenta banco", "value description listId").lean();
    res.json({
...
export const createPlanTrabajoMTM = async (req, res) => {
  try {
    ...
    const ctxPlanC = await loadMtmPostulacionContext(postulacionId);
    if (ctxPlanC) {
      await dispatchMonitoriaNotificacion("creacion_plan_trabajo_monitoria", ctxPlanC.datos, {
        docente: ctxPlanC.profEmail,
        coordinador: [ctxPlanC.profEmail, ...parseEnvEmailList(process.env.NOTIFICATION_EMAILS_COORDINADOR)].filter(
          Boolean
        ),
        administrador: parseEnvEmailList(process.env.NOTIFICATION_EMAILS_ADMIN),
      });
...
export const enviarRevisionPlanTrabajoMTM = async (req, res) => {
  try {
    ...
    if (ctxEnvPlan) {
      const revisionLink = `/dashboard/monitorias/revision/${String(postulacionId)}`;
      await dispatchMonitoriaNotificacion(
        "envio_revision_plan_trabajo_monitoria",
        {
          ...ctxEnvPlan.datos,
        ...
        {
          docente: ctxEnvPlan.profEmail,
          coordinador: ctxEnvPlan.profEmail ? [ctxEnvPlan.profEmail] : [],
        }
      );
    }
...
export const aprobarPlanTrabajoMTM = async (req, res) => {
  try {
    ...
    const ctxAprobPlan = await loadMtmPostulacionContext(postulacionId);
    if (ctxAprobPlan) {
      await dispatchMonitoriaNotificacion(
        "aprobacion_plan_trabajo_monitoria",
        buildVariablesPlantillaMtmPlanCargo({
          nombreCargo: plan.nombreMonitor || plan.asignaturaArea,
          periodoCodigo: plan.periodo,
        }),
        {
          coordinador: [ctxAprobPlan.profEmail, ...parseEnvEmailList(process.env.NOTIFICATION_EMAILS_COORDINADOR)].filter(
            Boolean
          ),
          administrador: parseEnvEmailList(process.env.NOTIFICATION_EMAILS_ADMIN),
        }
...
export const getSeguimientosMTM = async (req, res) => {
  try {
    ...
    if (isStudent) {
      ...
      /** Admin: no filtrar por `aceptado_estudiante` (datos migrados / listado legalización pueden tener otro estado). */
      const po = await PostulacionMTM.findOne({ _id: postulacionId }).lean();
      if (!po) return res.status(404).json({ message: "Postulación no encontrada" });
    }
...
export const createSeguimientoMTM = async (req, res) => {
  try {
    ...
    const ctxSegN = await loadMtmPostulacionContext(postulacionId);
    if (ctxSegN) {
      await dispatchMonitoriaNotificacion(
        "creacion_seguimiento_monitoria_requiere_aprobacion",
        {
          ...ctxSegN.datos,
        ...
        {
          docente: ctxSegN.profEmail,
          coordinador: [ctxSegN.profEmail, ...parseEnvEmailList(process.env.NOTIFICATION_EMAILS_COORDINADOR)].filter(
            Boolean
          ),
          administrador: parseEnvEmailList(process.env.NOTIFICATION_EMAILS_ADMIN),
        }
...
export const aprobarSeguimientoMTM = async (req, res) => {
  try {
    ...
    await seg.save();
    const updated = await SeguimientoMTM.findById(seg._id).populate("creadoPor", "name email").populate("aprobadoPor", "name email").lean();
    const ctxSegA = await loadMtmPostulacionContext(postulacionId);
    if (ctxSegA) {
      await dispatchMonitoriaNotificacion(
        "aprobacion_seguimiento_monitoria",
        {
          ...ctxSegA.datos,
          TIPO_ACTIVIDAD: seg.tipoActividad || "",
          HORAS: seg.cantidadHoras != null ? String(seg.cantidadHoras) : "",
        ...
        {
          estudiante: ctxSegA.estudianteEmail,
          postulante: ctxSegA.estudianteEmail,
          monitor_academico: ctxSegA.estudianteEmail,
        }
      );
    }
...
export const rechazarSeguimientoMTM = async (req, res) => {
  try {
    ...
    const ctxSegR = await loadMtmPostulacionContext(postulacionId);
    if (ctxSegR) {
      const motivoTxt = (seg.rechazoMotivo || "").trim();
      await dispatchMonitoriaNotificacion(
        "rechazo_seguimiento_monitoria",
        {
          ...ctxSegR.datos,
          TIPO_ACTIVIDAD: seg.tipoActividad || "",
          COMENTARIO: motivoTxt,
          OBSERVACION: motivoTxt,
        },
        { estudiante: ctxSegR.estudianteEmail, postulante: ctxSegR.estudianteEmail }
      );
    }
...
export const accionMasivaSeguimientosMTM = async (req, res) => {
  try {
    ...
    const ctxMas = await loadMtmPostulacionContext(postulacionId);
    if (ctxMas) {
      for (const s of segs) {
        const datosSeg = {
          ...ctxMas.datos,
          TIPO_ACTIVIDAD: s.tipoActividad || "",
          HORAS: s.cantidadHoras != null ? String(s.cantidadHoras) : "",
          FECHA: s.fecha ? new Date(s.fecha).toISOString().slice(0, 10) : "",
        };
        if (accion === "aprobar") {
          await dispatchMonitoriaNotificacion("aprobacion_seguimiento_monitoria", datosSeg, {
            estudiante: ctxMas.estudianteEmail,
            postulante: ctxMas.estudianteEmail,
            monitor_academico: ctxMas.estudianteEmail,
          });
        } else {
          const m = (motivo || "").toString().trim();
          await dispatchMonitoriaNotificacion(
            "rechazo_seguimiento_monitoria",
            { ...datosSeg, COMENTARIO: m, OBSERVACION: m },
            { estudiante: ctxMas.estudianteEmail, postulante: ctxMas.estudianteEmail }
          );
        }
      }
    }
...
export const getOrCreateLinkAsistenciaMTM = async (req, res) => {
  try {
    ...
    const po = await PostulacionMTM.findOne(
      isStudent ? { _id: postulacionId, estado: "aceptado_estudiante" } : { _id: postulacionId }
    );
    if (!po) {
      return res.status(404).json({
        message: isStudent ? "Postulación no encontrada o no aceptada" : "Postulación no encontrada",
      });
    }
    let linkRecienCreado = false;
    if (!po.linkAsistenciaToken) {
      po.linkAsistenciaToken = crypto.randomUUID();
      await po.save();
      linkRecienCreado = true;
    }
    const baseUrl = process.env.FRONTEND_URL || process.env.CLIENT_URL || "http://localhost:5173";
    const link = `${baseUrl.replace(/\/$/, "")}/#/asistencia-mtm/${po.linkAsistenciaToken}`;
    if (linkRecienCreado) {
      const ctxAsis = await loadMtmPostulacionContext(postulacionId);
      if (ctxAsis) {
        await dispatchMonitoriaNotificacion(
          "envio_link_asistencia_monitor",
          { ...ctxAsis.datos, LINK: link },
          { estudiante: ctxAsis.estudianteEmail, postulante: ctxAsis.estudianteEmail }
        );
      }
    }
...
/** GET reporte de asistencia por postulación para admin/coordinación (sin validación de ownership estudiante). */
